...
(function (MatterObjects) {
    ...
    function makeWalls(x = 0, y = 0, w = 1000, h = 600, thick = 200) {
        let left = Bodies.rectangle(x, y + h / 2, thick, h, { label: 'ground' });
        let right = Bodies.rectangle(x + w, y + h / 2, thick, h, { label: 'lwall' });
        let floor = Bodies.rectangle(x + w / 2, y + h, w, thick, { label: 'rwall' }); // small overlap of 5 to ensure there isn't a gap
        let ceil = Bodies.rectangle(x + w / 2, y, w, thick, { label: 'ceil' });
    ...
    MatterObjects.mouseConstraint = Matter.MouseConstraint.create(engine, {
        // @ts-ignore
        canvas: canva,
        mouse: mouse,
        constraint: {

...
describe("tests the inspector class", function () {
    it("correctly picks up attributes of substances", () => {
        ...
        expect(allKeys(new Substance())).toEqual(['substances', 'subsystems', 'type', 'state', 'mass', 'volume', 'temperature', 'kValue', 's']);
    });
    it("dynamically walks the proto chain", function () {
        ...
        expect(traceExtensionsOn(new (makeGaseous(AqueousSubstance()))()).map(x => x.name))
